...
namespace cnpy
{
  ...
  template<typename T> std::vector<char>& operator+=(std::vector<char>& lhs, const T rhs)
  {
    ...
    for (char byte = 0; byte < (int) sizeof(T); byte++)
    {
  ...
  template<typename T> void npy_save(std::string fname, const T* data, const unsigned int* shape, const unsigned int ndims,
                                     std::string mode = "w")
  {
    FILE* fp = NULL;
  ...
  template<typename T> void npz_save(std::string zipname, std::string fname, const T* data, const unsigned int* shape,
